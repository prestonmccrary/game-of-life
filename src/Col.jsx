...
const Col = ({squares, squareSize, callback}) => {

    return(
        <div style={{height: squares.length * squareSize, width: squareSize, display: 'flex', flexDirection: 'column'}}>
            {squares.map((bool, index) => {
                return(
                    <div clas style={{cursor: bool ? 'pointer': '',display: 'flex',justifyContent: 'center',alignItems:'center',width: squareSize, height: squareSize, border: '#e0e0e0 0.1px solid'}} onClick={() => callback(index)}> 
